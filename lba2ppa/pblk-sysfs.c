...
static ssize_t pblk_sysfs_get_l2ptable(struct pblk *pblk,const char *page)
{
	int sz = 0;
...
/*
	...
	fd = sys_open ("/etc/l2p", O_WRONLY | O_CREAT, 0644);
	...
	if (fd >= 0){
		...
		printk("%d",file);
		if(file){
			snprintf(buff,sizeof(buff),"kdy test\n");
			vfs_write(file,buff,strlen(buff), &pos);*/
			for(lba=1;lba<500000;lba++){
			//for(lba=1;lba<=100;lba++){
				//snprintf(buff,sizeof(buff),"%lu %lu\n",lba,pblk_ppa64_to_ppa32(pblk,pblk_trans_map_get(pblk,lba)));
				//vfs_write (file, buff, strlen(buff), &pos);
				//fput (file);
				trace_printk("%lu %u\n",lba,pblk_ppa64_to_ppa32(pblk,pblk_trans_map_get(pblk,lba)));
			}/*
...
static struct attribute sys_get_l2ptable = {
	.name = "get_l2ptable",
	.mode = 0444,
};
		...
		char *buf)
{
	...
	else if (strcmp(attr->name, "get_l2ptable") == 0)
		return pblk_sysfs_get_l2ptable(pblk,buf);
#ifdef CONFIG_NVM_PBLK_DEBUG
